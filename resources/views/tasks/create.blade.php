...
@section('content')
    <div class="mb-4">
        <h2 class="text-2xl font-semibold">新規タスク作成</h2>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow">
        <form action="{{ route('tasks.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300" required>
                @error('title')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="due_date" id="due_date" value="{{ old('due_date') }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300">
                @error('due_date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="relative" x-data="{ open: false, selectedId: '{{ old('category_id', '') }}', selectedName: '{{ old('category_id') ? '' : 'カテゴリなし' }}' }">
                    <!-- カスタムセレクトの表示部分 -->
                    <div
                        @click="open = !open"
            ...
            <div class="flex items-center justify-between">
                ...
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">作成する</button>
            </div>
        </form>
    </div>
@endsection
